...
## Prereqs
\small
This tutorial uses the NFLcombine dataset that came from NFLSavant.com. This is a database consisting of NFL Combine data from 1999 to 2015 and is documented in ?NFLcombine. This tutorial also requires the following packages:
...
# Added to update to AWS vignette
#devtools::install_github("h-greggc/DecisionAnalysis", force = TRUE)
...
value_hierarchy_tree(branches$Level1, branches$Level2, branches$Level3,   leaves=branches$leaves, nodefillcolor = "LightBlue", leavesfillcolor = "Blue", leavesfontcolor = "White")
...
a1 <- SAVF_exp_plot(90, 0, 120, 150)
a2 <- SAVF_linear_plot(10, 0, 20, 100, FALSE)
a3 <- SAVF_cat_plot(c("Tom", "Bill", "Jerry"), c(0.1, 0.25, 0.65))
...
```{r}
Height <- round(SAVF_exp_score(qbdata$Height , 68, 75.21, 82), 3)
Weight <- round(SAVF_exp_score(qbdata$Weight, 185, 224.34, 275), 3)
Forty <- round(SAVF_exp_score(qbdata$Forty, 4.3, 4.81, 5.4, FALSE), 3)
Shuttle <- round(SAVF_exp_score(qbdata$Shuttle, 3.8, 4.3, 4.9, FALSE), 3)
Vertical <- round(SAVF_exp_score(qbdata$Vertical, 21, 32.04, 40), 3)
Broad <- round(SAVF_exp_score(qbdata$Broad, 90, 111.24, 130), 3)
Wonderlic <- round(SAVF_exp_score(qbdata$Wonderlic, 0, 27.08, 50), 3)
...
MAVF <- MAVF_Scores(SAVF_matrix, weights, names)
...
Height <- SAVF_exp_score(qbdata$Height, 68, 75.21, 82, 1)
Weight <- SAVF_exp_score(qbdata$Weight, 185, 224.34, 275, 1)
Forty <- SAVF_exp_score(qbdata$Forty, 4.3, 4.81, 5.4, 2)
Shuttle <- SAVF_exp_score(qbdata$Shuttle, 3.8, 4.3, 4.9, 2)
Vertical <- SAVF_exp_score(qbdata$Vertical, 21, 32.04, 40, 1)
Broad <- SAVF_exp_score(qbdata$Broad, 90, 111.24, 130, 1)
Wonderlic <- SAVF_exp_score(qbdata$Wonderlic, 0, 27.08, 50, 1)
